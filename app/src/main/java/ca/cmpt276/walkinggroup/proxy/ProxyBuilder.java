...
public class ProxyBuilder {
     ...
     * @param apiKey     Your group's API key to communicate with the server.
     * @param token      The token you have been issued
     * @param jsonDepth  The depth of the returned json content
     * @return proxy object to call the server.
     */
    public static WGServerProxy getProxy(String apiKey, String token, int jsonDepth) {
        // Enable Logging
        HttpLoggingInterceptor logging = new HttpLoggingInterceptor();
        logging.setLevel(HttpLoggingInterceptor.Level.BODY);
        OkHttpClient client = new OkHttpClient.Builder()
    ...
    private static class AddHeaderInterceptor implements Interceptor {
        ...
        public AddHeaderInterceptor(String apiKey, String token, int jsonDepth){
            this.apiKey = apiKey;
            this.token = token;
        ...
        public Response intercept(Interceptor.Chain chain) throws IOException {
            ...
            // Add Json Depth
            if (Integer.toString(jsonDepth) != null){
                builder.header("JSON-DEPTH", Integer.toString(jsonDepth));
            }
